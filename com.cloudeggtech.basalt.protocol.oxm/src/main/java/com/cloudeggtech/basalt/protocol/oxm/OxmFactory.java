...
public class OxmFactory implements IOxmFactory {
	...
	private ProtocolException newInternalServerErrorException(Exception e, boolean stream) {
		...
		if (stream) {
			return new ProtocolException(new com.cloudeggtech.basalt.protocol.core.stream.error.InternalServerError(), e);
		} else {
			return new ProtocolException(new com.cloudeggtech.basalt.protocol.core.stanza.error.InternalServerError(), e);
		}
	}
