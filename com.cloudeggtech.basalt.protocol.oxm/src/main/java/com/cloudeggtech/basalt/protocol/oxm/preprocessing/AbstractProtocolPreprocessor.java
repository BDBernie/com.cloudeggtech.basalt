...
import com.cloudeggtech.basalt.protocol.core.stream.error.XmlNotWellFormed;

public abstract class AbstractProtocolPreprocessor implements IProtocolPreprocessor {
	...
	public AbstractProtocolPreprocessor() {
		maxBufferSize = getDefaultMaxBufferSize();
	}
	
	@Override
	public String[] parse(char[] bytes, int readBytes) throws OutOfMaxBufferSizeException, ProtocolException {
		resetBuffer();
		
		if (buffer.length > maxBufferSize) {
			throw new OutOfMaxBufferSizeException("Stop parsing never ending message.");
		}
		
		buffer = appendBytesToBuffer(bytes, readBytes);
	...
	protected char[] appendBytesToBuffer(char[] bytes, int readBytes) {
		char[] newBuffer = new char[buffer.length + readBytes];
		
		System.arraycopy(buffer, 0, newBuffer, 0, buffer.length);
		System.arraycopy(bytes, 0, newBuffer, buffer.length, readBytes);
